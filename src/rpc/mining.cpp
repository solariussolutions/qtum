...
UniValue generateBlocks(boost::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    {
    	bool fStake = nHeight + 1 >= Params().LastPOWBlock();
    	int64_t nFees;
    	BlockAssembler blckAsm(Params());
		std::unique_ptr<CBlockTemplate> pblocktemplate(blckAsm.CreateNewBlock(coinbaseScript->reserveScript, fStake, &nFees));
        ...
        CBlock *pblock = &pblocktemplate->block;
        {
            ...
            if (fStake)
			{
				if (SignBlock(*pblock, *pwalletMain, nFees, blckAsm.voutCoinBaseTX, blckAsm.usedFee))
				{
					if (!CheckStake(pblock, *pwalletMain))
						continue;
				} else {
                    continue;
                }
...
UniValue generatetoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. numblocks    (numeric, required) How many blocks are generated immediately.\n"
            "2. address    (string, required) The address to send the newly generated quantum to.\n"
    ...
    int nGenerate = params[0].get_int();
    uint64_t nMaxTries = 0x100000000;
    if (params.size() > 2) {
...
UniValue getstakesubsidy(const UniValue& params, bool fHelp)
{
    ...
    if (!GetCoinAge(tx, *pblocktree, pindexBestHeader))
        throw JSONRPCError(RPC_MISC_ERROR, "GetCoinAge failed");

    return (uint64_t)GetProofOfStakeReward();
}
//end modif qtum
...
// NOTE: Unlike wallet RPC (which use QTUM values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/quantum/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/quantum/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
...
UniValue submitblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "See https://en.quantum.it/wiki/BIP_0022 for full specification.\n"
