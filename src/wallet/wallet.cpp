...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CQuantumAddress(CScriptID(redeemScript)).ToString();
...
bool CWallet::SelectCoinsMinConf(const CAmount& nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins,
...
//end modif qtum
{
    ...
    bool res = nTargetValue <= nValueFromPresetInputs ||
        ...
        (bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue - nValueFromPresetInputs, nSpendTime, 0, 1, vCoins, setCoinsRet, nValueRet));
        //end modif qtum
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-quantum-address
                ...
                BOOST_FOREACH(const PAIRTYPE(const CWalletTx*,unsigned int)& coin, vCoins)
                ...
                *static_cast<CTransaction*>(&wtxNew) = CTransaction(txNew);
                ...
                CAmount nFeeNeeded = GetMinimumFee(nBytes, nTxConfirmTarget, mempool)+nGasFee;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CAmount& nFees, CMutableTransaction& tx, CKey& key, const vector<CTxOut>& refundOuts, const CAmount usedGas)
{
    ...
    // Calculate reward
    {
        if (!GetCoinAge(txNew, *pblocktree, pindexPrev))
            return error("CreateCoinStake : failed to calculate coin age");

        int64_t nReward = nFees + GetProofOfStakeReward();
        if (nReward <= 0)
            return false;
	...
	for(unsigned int i = 0; i < refundOuts.size(); i++){
		txNew.vout.push_back(refundOuts[i]);
	}
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CQuantumAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CQuantumAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CQuantumAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CQuantumAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CQuantumAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CQuantumAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CQuantumAddress(dest).ToString(), key);
}
