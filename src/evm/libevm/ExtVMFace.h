...
#include <libqtumevm/QuantumEVM.h>
...
public:
   ...
   EnvInfo(CBlock const& _current, CChain* _chain, u256 const& _gasUsed = u256()):
      m_number(dev::u256(_chain->Tip()->nHeight + 1)),
      m_timestamp(dev::u256(_current.nTime)),
      m_difficulty(dev::u256(_current.nBits)),
      m_gasUsed(_gasUsed)
   {
      CBlockIndex* tip = _chain->Tip();
