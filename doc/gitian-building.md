...
*Setup instructions for a Gitian build of Quantum Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Quantum
...
- [Building Quantum Core](#building-quantum-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-quantum-host/README.md).
...
Clone the git repositories for quantum and Gitian.
...
in the quantum repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Quantum Core
----------------

To build Quantum Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the quantum repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/quantum/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/quantum/quantum
...
For example:
```bash
URL=https://github.com/laanwj/quantum.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit quantum=${COMMIT} --url quantum=${URL} ../quantum/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit quantum=${COMMIT} --url quantum=${URL} ../quantum/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit quantum=${COMMIT} --url quantum=${URL} ../quantum/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the quantum git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../quantum/contrib/gitian-descriptors/*|sort|uniq )
...
QTUMPATH=/some/root/path/quantum
SIGPATH=/some/root/path/quantum-detached-sigs

./bin/gbuild --url quantum=${QTUMPATH},signature=${SIGPATH} ../quantum/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/quantum-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/quantum-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/quantum-osx-build.assert
```
...
[quantum-core/gitian.sigs](https://github.com/quantum-core/gitian.sigs/) repository, or if that's not possible create a pull
