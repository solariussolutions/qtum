...
The RPM spec file provided here is for Quantum-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://quantum.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Quantum git repository](https://github.com/quantum/quantum) but are not part of
...
Currently only `Source10` is used, for the example `quantum.conf` file.
...
in the quantum git repository. At present only an SVG version of the Quantum
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the quantum
...
needed when 0.12.1 is released, a proper fix is already in the Quantum git
master branch.
...
idea to build Quantum Core against a static version of that library compiled
...
The `quantum.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `quantum-server` package. At this point, most RPM based
...
The `quantum-server` package should have SELinux support. How to properly do
...
Quantum-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/quantum/). He has been
packaging Quantum for Fedora at least since 2012.
...
4. I renamed the `quantum` package that contains the Qt GUI to `quantum-core` as
that appears to be how the general population refers to it, in contrast to
`quantum-xt` or `quantum-classic`. I wanted to make sure the general population
