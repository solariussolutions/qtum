...
###   This script attempts to download the signature file SHA256SUMS.asc from quantum.org
...
WORKINGDIR="/tmp/quantum"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://quantum.org/bin/"
VERSIONPREFIX="quantum-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'quantum-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Quantum Core binary release signing key installed?"
   fi
